...
const indexDbResponse$ = new Subject();
const indexDbResponse = indexDbResponse$.asObservable();
...
class LocalDb$ {
  ...
  connect() {
    return new Promise(callback => {
      ...
      request.onupgradeneeded = event => upgradeDb(event, this.definitions, callback);
    });
  }
...
const asyncLoop = async (rows, action) => {
  for (var row, i = 0; row = rows[i++]; await percentDb(rows.length, i, () => action(row)));
};

const upgradeDb = (event, definitions, callback) => {
  const db = event.target.result;
  definitions.map(definition => {
    try { db.deleteObjectStore(definition.name); } catch (e) { console.log(e) }
