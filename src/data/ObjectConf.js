...
const DASH_DATA_CONFIG = [
  {
    ...
    table: "localDbArtists",
    parse: (data) =>
      randomize(data.filter((f) => f.trackCount > 25)).slice(0, 10),
  },

  {
    ...
    table: "localDbAlbums",
    parse: (data) =>
      randomize(data.filter((f) => f.trackCount > 9)).slice(0, 10),
  },
  {
    label: "Recently Added",
    table: "localDbMusic",
    parse: (data) =>
      randomize(data.filter((f) => f.albumImage?.length && !!f.artistFk)).slice(
        0,
        99
      ),
  },
];
const INDEX_NAME = "react_amplify_local_indexed_psuedo_db_with_cache";
const DB_VERSION = 1;
